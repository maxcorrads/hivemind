...
import { exportRun, prepareRun, runStudy, sha256 } from './topology-study-runner.mjs';
...
function setup({ wallMs = 20_000, expectedResolvedModel = null, first = null } = {}) {
  ...
  const setBehavior = (behavior, tokens = 40) => writeFileSync(control, JSON.stringify({ behavior, tokens }));
  setBehavior('complete');
  const run = JSON.parse(readFileSync(path.join(runDir, 'run.json'), 'utf8'));
  const host = (serverEnv = {}) => createHivemindHost({ repoRoot, plan: run, evidenceKind: 'synthetic',
    ...
    joinTimeoutMs: 20_000, pollMs: 100, acceptanceTimeoutMs: 10_000 });
  const execute = (options = {}) => runStudy({ runDir, host: host(options.serverEnv), authorization: study.studyId, maxTrials: options.maxTrials ?? Infinity });
  return { dir, runDir, study, setBehavior, execute, cleanup: () => rmSync(dir, { recursive: true, force: true }) };
}
