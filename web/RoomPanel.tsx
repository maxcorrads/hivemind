...
export function RoomPanel({ channel, agents, tick }: { channel: Channel; agents: Agent[]; tick: number }) {
  ...
  const submit = async (action: unknown, expectedRevision: number) => {
    ...
    try {
      await api.roomEvent(channel.id, { requestId: crypto.randomUUID(), expectedRevision, action });
  ...
  return <section className="room-panel" aria-label="Channel contract">
    <div className="room-panel-heading"><strong>Channel contract</strong>
      {!editing && room?.state !== 'archived' && <button type="button" disabled={!view || busy} onClick={begin}>{room ? 'Edit contract' : 'Set up contract'}</button>}
    </div>
    ...
    {room && !editing && <div className="room-controls">
      {room.state === 'active' && !archiving && <button disabled={busy} onClick={() => { setBase(room.revision); setReason(''); setError(''); setRunning(''); setArchiving(true); }}>Archive channel…</button>}
