...
export function RoomDetails({ view }: { view: RoomView }) {
  ...
  if (!room) return null;
  return <>
    <p><strong>{room.contract.mode === 'finite' ? 'Task-scoped room' : 'Ongoing channel'} · {room.state}</strong> · revision {room.revision} / contract {room.contractVersion}</p>
    <p>Coordinating brain: {room.contract.coordinator}</p>
    <p>{room.contract.purpose}</p>
    ...
    <details><summary>Participants, completion and task state</summary>
    ...
    {(view.links.length > 0 || view.unmanagedBots.length > 0) && <details open={room.state === 'archived'}><summary>Source suspension reports</summary>
...
export function RoomPanel({ channel, agents, tick }: { channel: Channel; agents: Agent[]; tick: number }) {
  ...
  return <section className="room-panel" aria-label="Channel contract">
    <div className="room-panel-heading"><strong>Channel contract</strong>
      {!editing && room?.state !== 'archived' && <button type="button" disabled={!view || busy || !!pending} onClick={begin}>{room ? 'Edit contract' : 'Set up contract'}</button>}
    </div>
    ...
    {pending && <button type="button" disabled={busy} onClick={() => void submit(pending.action, pending.expectedRevision)}>Retry exact request</button>}
    {(pending || error) && <button type="button" disabled={busy} onClick={() => void reconcile()}>Reconcile with latest state</button>}
    ...
    {editing && draft && <form onSubmit={e => { e.preventDefault(); void submit({ type: 'configure', contract: {
      ...
      <fieldset><legend>Invited workers and ownership boundaries</legend>{workers.map(w => {
        ...
        return <div key={w.id}><label><input type="checkbox" checked={Boolean(p)} onChange={e => setDraft({ ...draft, participants: e.target.checked ? [...draft.participants, { name: w.name, boundary: '' }] : draft.participants.filter(p => p.name !== w.name) })} />{w.name}</label>
      ...
      <button disabled={busy} type="submit">Save contract</button> <button disabled={busy} type="button" onClick={() => { setEditing(false); setError(''); }}>Cancel</button>
      </fieldset>
    </form>}
    {room && !editing && <div className="room-controls">
      {room.state === 'active' && !archiving && <button disabled={busy || !!pending} onClick={() => { setBase(room.revision); setReason(''); setError(''); setRunning(''); setArchiving(true); }}>Archive channel…</button>}
      {archiving && <form onSubmit={e => { e.preventDefault(); void submit({ type: 'archive', reason, ...(hasRunning ? { running } : {}) }, base); }}>
        ...
        <button disabled={busy} type="submit">Confirm archive</button> <button disabled={busy} type="button" onClick={() => setArchiving(false)}>Cancel</button>
        </fieldset>
      </form>}
      {room.state === 'archived' && <form onSubmit={e => { e.preventDefault(); void submit({ type: 'reopen', reason, resumeSources }, room.revision); }}>
        ...
        <label><input type="checkbox" checked={resumeSources} onChange={e => setResumeSources(e.target.checked)} />Request resumption of registered source links</label>
        <p>Stopped tasks stay stopped. Old observations are not automatically replayed.</p>
        <button disabled={busy} type="submit">Reopen channel</button>
        </fieldset>
      </form>}
      <button disabled={busy} type="button" onClick={() => api.roomHistory(channel.id).then(r => { if (mounted.current) setHistory(r.history); }).catch(e => setError(e.message))}>Show recent contract history</button>
      {history.length > 0 && <details open><summary>Latest {history.length} revisions</summary>{history.map(r => <div key={r.revision}><strong>Revision {r.revision}, contract {r.contractVersion} · {r.state}</strong><p>{r.contract.purpose}</p><p>{r.contract.rules.join('; ')}</p><p>{r.changedBy?.name} ({r.changedBy?.role}): {r.changedBy?.reason}</p><p>Human instruction: {r.humanInstructionSeq ?? 'direct Human configuration'}</p></div>)}</details>}
    </div>}
  </section>;
}
