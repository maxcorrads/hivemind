...
/**
 * Structured task facts: state, objective and progress up top, the key facts in a well below, then the contract,
 * claim, checkpoint, result, routing and timeline behind disclosures.
 */
export function TaskCard({ task, now }: { task: TaskSnapshot; now?: number }) {
  ...
  const branch = task.contract.branch ?? task.checkpoint?.branch;
  return <section className="task-card" aria-label="Structured task">
    <div className="task-card-main">
      <header>
        <TaskChip state={task.state} />
        <small className="task-rev">rev {task.revision} · contract {task.contractVersion}</small>
      ...
      {task.cancellation && <p role="status" className="task-cancelled">Cancelled: {task.cancellation.reason}. The assigning brain can reassign it with a revise.</p>}
    </div>
    <dl className="task-facts">
      <dt>Branch</dt><dd className="mono">{branch ?? 'not specified'}</dd>
      <dt>Acceptance</dt><dd>{task.contract.acceptanceCriteria.join(' · ') || 'none specified'}</dd>
      <dt>Receipt</dt><dd>{task.receivedAt !== null
        ? <>Worker confirmed receipt <RelativeTime at={task.receivedAt} now={now} /></> : 'Worker receipt not confirmed'}</dd>
      {task.room && <><dt>Room</dt><dd role="status">Room contract {task.room.contractVersion} / current {task.room.currentVersion} · {task.room.status.replaceAll('_', ' ')} · {task.room.acknowledged ? 'Rules acknowledged' : 'Rules not yet acknowledged'}</dd></>}
      <dt>Review</dt><dd>{task.review
        ? <>Assigning brain review: {task.review.decision.replaceAll('_', ' ')} — {task.review.summary}</> : 'No review decision yet.'}</dd>
    </dl>
    <div className="task-sections">
      <details><Disclosure>Contract and evidence</Disclosure>
        ...
        <small className="mono">Task {task.id}</small>
      </details>
      {task.coordination && task.coordination.dependencies.length > 0 && <details open><Disclosure>Prerequisite status at last refresh</Disclosure>
        {task.coordination.dependencies.map(dependency => <p key={dependency.taskId}>{dependency.taskId}: {dependency.status.replaceAll('_', ' ')}</p>)}
      </details>}
      {task.claim && <details open className="task-claim"><Disclosure>Advisory claim · {claimState(task)}</Disclosure>
      ...
      {task.checkpoint && <details className="task-handoff" open><Disclosure>Latest checkpoint · version {task.checkpoint.version}</Disclosure>
      ...
      {task.result && <details open><Disclosure>Reported result</Disclosure>
