 ...
 * change when the stream re-renders. `grouped` and `taskRoute` are primitives.
 */
export function MessageRow({ m, replies, status, grouped, taskRoute, onThread, onReact, onMarkUnread }: {
  m: Message;
  replies: number;
  ...
  return <Msg m={m} replies={replies} status={status} grouped={grouped} taskRoute={taskRoute}
