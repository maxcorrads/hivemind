...
type Pending = { channel: string; body: string; root: string | null; files: File[]; routing: SendRoutingMode;
...
export function createSendOperations<T>(upload: (file: File) => Promise<{ id: string }>,
  send: (channel: string, body: string, root: string | null, ids: string[], requestId: string, routing: SendRoutingMode) => Promise<T>) {
  const pending: Pending[] = [];
  return (channel: string, body: string, root: string | null, files: File[] = [], routing: SendRoutingMode = "auto"): Promise<T> => {
    let operation = pending.find(p => p.channel === channel && p.body === body && p.root === root && p.routing === routing &&
    ...
    if (!operation) {
      ...
      operation = { channel, body, root, files: [...files], routing, ids: [], key: crypto.randomUUID(), at: Date.now() };
    ...
    current.running = (async () => {
      ...
      const result = await send(channel, body, root, [...current.ids], current.key, current.routing);
