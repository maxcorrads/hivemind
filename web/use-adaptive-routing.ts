...
export function useAdaptiveRouting(channelId: string | null) {
  ...
  // Observations (no single owning brain) carry an audit event without a request state.
  ...
  return { view: stored?.channelId === channelId ? stored.view : null, error, refresh, onEvent };
}
