...
If you already ran `npm run build` (web UI into `dist/web`, compiled CLI/server/MCP into `dist/node`), the UI is also on `7420`; the packaged `hivemind` binary runs that compiled JavaScript without `tsx`. Local production: `npm run build && npm start`. Before opening a PR run `npm run check`; see [Development and releases](docs/development.md).
