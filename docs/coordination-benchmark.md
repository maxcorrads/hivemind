...
Prompt version `coordination-real-v4` / task version `coordination-task-v2` gives each task a deterministic SHA-256 artifact contract and supports information-partitioned fixtures whose private worker facts are absent from the shared runbook. The focused `clarification-v1` preset runs exactly six trials (single, multi-DM, multi-room × two repeats) to test whether real clarification can justify orchestration overhead. The generic `benchmark:coordination:run` command executes Codex or OpenCode versioned presets with isolated Hivemind state and one external model process per actual workflow seat; native host subagents stay disabled so the measured coordination shape remains Hivemind's. Local executable aliases are supplied only through `CODEX_BIN` / `OPENCODE_BIN` and are not stored in the repository. See `docs/coordination-real-agent.md`.
