...
- One focused `macOS compatibility` job runs the launch/bot shell contracts with native macOS zsh. Linux uses bash for the cross-platform shell argument contracts and does not install zsh on every ephemeral runner; zsh-only syntax validation remains native to macOS.
