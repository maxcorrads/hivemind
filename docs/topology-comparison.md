...
- Distinct Jev contract versions cannot be pooled: `adaptive-routing-v3` (joint plan, #207) and `adaptive-routing-v2` (separate topology and budget) evidence are both accepted, but only as separate cohorts; `validate` reports `contractVersions`, and a `mixed` export is refused.
