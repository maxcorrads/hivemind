...
configuration executable runs with your user permissions. The loopback Human API
requires the local browser session before Configure or availability changes.
This is a browser boundary, not OS isolation from hostile local programs; native
programs with local access can imitate bootstrap. See
[the extensibility security contract](EXTENSIBILITY-SECURITY.md).

Run `npm run check` and `npm run test:coverage` (both server and UI tests are discovered).
...
Configure runs a deliberately installed, trusted local executable without a shell,
with the project profile as its working directory. It inherits only basic OS
variables, not server service credentials or `NODE_OPTIONS`. Plugin credentials must
be stored separately from declarative settings. Error receipts and stdout/stderr
are never echoed to HTTP; use local CLI validation for detailed diagnostics.

At most eight updates are running/queued per process; excess requests return 429.
The 15-second command deadline and 64 KiB output limit remain. Ordinary remaining
process-group members are terminated when configuration exits; this is not a
sandbox or supervisor for intentionally detached daemons. Disable/unregister does
not terminate a monitor or revoke its bot token.

The Human API's browser/Host defenses do not authenticate arbitrary programs under
the same OS account. See [Extensibility security](EXTENSIBILITY-SECURITY.md) before
installing a plugin or forwarding a local Hivemind port.
