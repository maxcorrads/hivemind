...
In a scoped room, replying to a peer does not finish your own assigned task. When clarification resolves your blocker, continue that task and submit its result before returning to idle wait. Copy exact channelId/rootId/taskId values from tool results; never reconstruct identifiers. A failed send or task operation is not a delivered message or completed work: reread get_task/get_room or the relevant history, correct an invalid reference, and retry the intended operation. Preserve idempotency IDs for unchanged retries; a changed payload after a rejected validation needs a fresh request ID. If recovery is blocked, report the actual error to the coordinator instead of silently waiting. This is active error recovery, not idle polling.
