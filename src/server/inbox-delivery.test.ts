...
test("failed receipt backfill rolls back its marker and totals so a restart can retry", t => {
  ...
  assert.throws(() => new InboxDeliveryStore(db), /NOT NULL constraint failed: inbox_receipt_totals.acknowledged_messages/);
  assert.equal(parsed, 2, "failure occurs after the first group has been processed");
