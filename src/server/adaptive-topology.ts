...
import { HiveError, HUMAN_ID, PRESENCE_IDLE_MS, type Agent, type Message } from '../shared/types.ts';
...
export type AdaptiveCoordinationEvent = {
  ...
  /** Stable identity of the coordination event, not the Jev HTTP call. */
  eventId?: string;
};
...
type TaskRow = {
  id: string; channelId: string; assignerId: string; workerId: string; state: string;
  dispatchSeq?: number; lastEventSeq?: number; contract?: { dependencies?: string[] };
  claim?: { state?: string };
};
...
function publicState(stored: StoredExecution): AdaptiveExecutionState {
  const { recentEvents: _recent, confirmationHighCount: _high, lastEventId: _last, ...state } = stored;
  return state;
}
...
function policyOf(state: StoredExecution): TopologyPolicyState {
  return { applied: { topology: state.currentTopology, workers: state.workerBudget },
    ...
    confirmation: { target: state.confirmationTopology ? { topology: state.confirmationTopology, workers: state.confirmationWorkers ?? 0 } : null,
      count: state.confirmations, highCount: state.confirmationHighCount ?? 0 },
    eventsSinceChange: state.eventsSinceChange };
}
function cleanEvent(event: AdaptiveCoordinationEvent): AdaptiveCoordinationEvent {
  // Previous events retain coordination metadata, not a sliding transcript of agent messages.
...
export class AdaptiveTopologyRuntime {
  ...
  constructor(private hive: Hive) {
    hive.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS adaptive_topology_locks (
        channel_id TEXT PRIMARY KEY, topology TEXT NOT NULL, updated_at INTEGER NOT NULL);
  ...
  async stop(): Promise<void> {
    this.stopped = true;
    this.hive.bus.off('agent', this.onAgent);
  ...
  private queued<T>(key: string, work: () => Promise<T>): Promise<T> {
    ...
    // The bookkeeping promise must always resolve; a rejected detached finally promise is unhandled.
  ...
  private conversationLock(channel: string): AdaptiveTopology | null {
    ...
    const value = row?.topology;
    return typeof value === 'string' && ADAPTIVE_TOPOLOGIES.includes(value as AdaptiveTopology) ? value as AdaptiveTopology : null;
  }
  private conversationLockWrite(channel: string, topology: AdaptiveTopology | null) {
    if (topology === null) this.hive.db.prepare('DELETE FROM adaptive_topology_locks WHERE channel_id=?').run(channel);
      ...
      .run(channel, topology, Date.now());
  }
  private taskRows(projectId: string): TaskRow[] {
    return this.hive.db.prepare(`SELECT t.snapshot FROM task_records t JOIN channels c ON c.id=t.channel_id
      WHERE c.project_id=?`).all(projectId).map(row => JSON.parse(String(row.snapshot)) as TaskRow);
  }
  private currentTaskIds(state: Pick<StoredExecution, 'executionId'>): Set<string> {
    return new Set(this.hive.db.prepare('SELECT task_id FROM adaptive_topology_tasks WHERE execution_id=?')
      .all(state.executionId).map(row => String(row.task_id)));
  }
  private capacity(state: Pick<StoredExecution, 'projectId' | 'brainId' | 'executionId'>): TopologyCapacitySnapshot {
    const now = Date.now();
    const workers = this.hive.listAgents().filter(a => a.role === 'worker' && a.projectId === state.projectId);
    const tasks = this.taskRows(state.projectId);
    const ownIds = this.currentTaskIds(state);
    const unfinished = (task: TaskRow) => !['accepted_complete', 'rejected'].includes(task.state);
    const held = (task: TaskRow) => task.claim?.state === 'held';
    const occupied = tasks.filter(task => unfinished(task) || held(task));
    const own = occupied.filter(task => ownIds.has(task.id));
    const other = new Set(occupied.filter(task => !ownIds.has(task.id)).map(task => task.workerId));
    const ours = new Set(own.map(task => task.workerId));
    const live = (worker: Agent) => worker.online && now - worker.lastSeenAt < PRESENCE_IDLE_MS;
    const free = workers.filter(worker => live(worker) && !ours.has(worker.id) && !other.has(worker.id));
    const committed = workers.filter(worker => live(worker) && ours.has(worker.id) && !other.has(worker.id));
    const states = new Map(tasks.map(task => [task.id, task.state]));
    const active = own.filter(unfinished);
    const available = [...committed.map(worker => ({ ...worker, committed: true })), ...free.map(worker => ({ ...worker, committed: false }))]
      .map(({ id, name, seniority, focus, committed: used }) => ({ id, name, seniority, focus, committed: used }));
    return { workers: { total: workers.length, online: workers.filter(live).length,
      busyOther: workers.filter(worker => other.has(worker.id)).length, busyCurrent: committed.length,
      free: free.length, usableForExecution: available.length, available },
      activeTasks: active.length, activeWorkers: new Set(own.map(task => task.workerId)).size,
      blockers: active.filter(task => task.state === 'blocked').length,
      openDependencies: active.reduce((sum, task) => sum + (task.contract?.dependencies ?? []).filter(id => states.get(id) !== 'accepted_complete').length, 0),
      workstreams: active.length, unreconciledClaims: own.filter(held).length };
  }
  private snapshot(state: StoredExecution, event: AdaptiveCoordinationEvent, capacity = this.capacity(state)): TopologyEvaluationSnapshot {
    ...
    const request = this.hive.getMessageById(state.rootMessageId).body;
    return { request, project: { slug: project.slug, name: project.name },
      ...
      trigger: { ...cleanEvent(event), summary: event.summary?.trim().slice(0, 400) },
  ...
  private saveEvent(event: AdaptiveRoutingEvent) {
    ...
    this.hive.db.prepare(`DELETE FROM adaptive_topology_events WHERE channel_id=? AND rowid NOT IN
      (SELECT rowid FROM adaptive_topology_events WHERE channel_id=? ORDER BY rowid DESC LIMIT 500)`)
      .run(event.channelId, event.channelId);
  }
  private commit(state: StoredExecution, event: AdaptiveRoutingEvent) {
    immediateTransaction(this.hive.db, () => { this.save(state); this.saveEvent(event); });
    this.publish(state, event);
  }
  private publish(state: StoredExecution, event: AdaptiveRoutingEvent) {
    // This bus topic goes only to the authenticated Human websocket, not message delivery.
  ...
  private assertEventActor(actor: Agent, event: AdaptiveCoordinationEvent) {
    ...
    if (event.channelId) {
      ...
      if (!this.hive.canSeeChannel(actor, channel) || !this.hive.canPost(actor, channel))
        throw new HiveError(403, 'Cannot coordinate in this channel');
    }
    if (event.taskId) this.hive.tasks.get(actor, event.taskId);
    if (event.workerName) {
      const worker = this.hive.getAgentByName(event.workerName);
      if (!worker || worker.role !== 'worker' || worker.projectId !== actor.projectId)
        throw new HiveError(400, 'Select a worker in the current project');
    }
  }
  private resolveState(actor: Agent, event?: AdaptiveCoordinationEvent): StoredExecution | null {
    ...
    const rows = this.hive.db.prepare('SELECT snapshot FROM adaptive_topology_executions WHERE brain_id=? AND project_id=?')
      .all(brainId, actor.projectId);
  ...
  private enabledOrLocked(state: StoredExecution): boolean {
    return Boolean(loadAdaptiveRouting(this.hive.home)?.enabled || state.lockedTopology);
  }
  private async evaluateStable(state: StoredExecution, event: AdaptiveCoordinationEvent, config: AdaptiveRoutingFile) {
    let capacity = this.capacity(state);
    let decision: AdaptiveTopologyDecision | null = null;
    for (let attempt = 0; attempt < 3; attempt++) {
      const signature = JSON.stringify(capacity.workers.available);
      decision = await evaluateAdaptiveTopology(this.snapshot(state, event, capacity), config);
      const next = this.capacity(state);
      if (decision.providerStatus !== 'ok' || signature === JSON.stringify(next.workers.available)) return { decision, capacity: next };
      capacity = next;
    }
  ...
  private async revalidateStored(state: StoredExecution, event: AdaptiveCoordinationEvent): Promise<StoredExecution> {
    ...
    if (event.eventId && event.eventId === state.lastEventId) return state;
    const from = state.currentTopology;
    const revision = state.revision ?? 0;
    const evaluated = await this.evaluateStable(state, event, config);
    const latest = this.row(state.channelId);
    // A Human lock/new request or disabled setting wins over an in-flight classifier result.
    if (!latest || latest.executionId !== state.executionId || (latest.revision ?? 0) !== revision ||
      !loadAdaptiveRouting(this.hive.home)?.enabled) return latest ?? state;
    const { decision, capacity } = evaluated;
    const forced: TopologyTarget | null = state.lockedTopology ? {
      topology: state.lockedTopology,
      workers: state.lockedTopology === 'single' ? 0 : state.lockedTopology === state.currentTopology
        ? state.workerBudget : Math.max(minimumTopologyWorkers(state.lockedTopology), state.desiredWorkers ?? 0),
    } : null;
    const next = advanceTopologyPolicy(policyOf(state), {
      target: { topology: decision.targetTopology, workers: decision.targetWorkers },
      confidence: decision.confidence, available: decision.providerStatus === 'ok',
    }, safetyOf(capacity), forced);
    ...
    state.confirmationTopology = next.confirmation.target?.topology ?? null;
    state.confirmationWorkers = next.confirmation.target?.workers ?? null;
    state.eventsSinceChange = next.eventsSinceChange;
    ...
    if (decision.providerStatus === 'ok' && capacity.workers.usableForExecution === 0 &&
      (decision.needsOrchestration || state.orchestratedOnly)) {
    ...
    const item = this.record(state, decision, from, next.changed ? 'transition' : state.warning ? 'warning' : 'evaluation', next.changed);
    this.commit(state, item);
    return state;
  }
  ...
  private async revalidateProject(projectId: string, event: AdaptiveCoordinationEvent) {
  ...
  private assertDelegation(state: StoredExecution, event: AdaptiveCoordinationEvent) {
    ...
    const policy = compactPolicy(state);
    if (policy.delegationPaused) throw new HiveError(409, 'Adaptive de-escalation pending; finish existing work before new delegation');
    if (state.currentTopology === 'single') throw new HiveError(409, 'Adaptive routing retains Single; new delegation is blocked');
    if (event.usesRoom === true && state.currentTopology !== 'brain_multi_room')
      throw new HiveError(409, `Applied topology ${state.currentTopology} does not permit new room work`);
    if (event.usesRoom === false && state.currentTopology === 'brain_multi_room')
      throw new HiveError(409, 'Applied topology is Room; new work must use the room contract');
    const worker = event.workerName ? this.hive.getAgentByName(event.workerName) : null;
    if (worker) {
      const capacity = this.capacity(state);
      const candidate = capacity.workers.available.find(w => w.id === worker.id);
      if (!candidate) throw new HiveError(409, 'Selected worker is not available for this execution');
      if (!candidate.committed && capacity.activeWorkers >= state.workerBudget)
        throw new HiveError(409, `Applied worker budget is ${state.workerBudget}; no additional worker slot is available`);
    }
  }
  async beforeBrainAction(actor: Agent, event: AdaptiveCoordinationEvent): Promise<AdaptiveAgentPolicy | null> {
    ...
    const hash = this.hive.db.prepare('SELECT token_hash FROM agents WHERE id=?').get(actor.id)?.token_hash;
    const state = await this.revalidateForActor(actor, event);
    if (hash !== this.hive.db.prepare('SELECT token_hash FROM agents WHERE id=?').get(actor.id)?.token_hash)
      throw new HiveError(401, 'Credentials changed during routing; rejoin before retrying');
  ...
  async afterAgentAction(actor: Agent, event: AdaptiveCoordinationEvent): Promise<AdaptiveAgentPolicy | null> {
    ...
    const state = this.resolveState(actor, event);
    if (!state || !this.enabledOrLocked(state)) return null;
    if (event.taskId && actor.role === 'brain' && event.eventType === 'assignment') {
      const task = this.hive.tasks.get(actor, event.taskId);
      if (task.assignerId === actor.id) this.hive.db.prepare(
        'INSERT OR IGNORE INTO adaptive_topology_tasks(task_id,execution_id) VALUES(?,?)').run(task.id, state.executionId);
    }
    // Brain mutations were already evaluated at their mandatory pre-action boundary.
    // A second post-action call is not a second independent confirmation.
    if (actor.role === 'brain') return compactPolicy(this.row(state.channelId) ?? state);
  ...
  private manualBudget(topology: AdaptiveTopology, proposed: number, capacity: AdaptiveWorkerCapacity): number {
    ...
    return Math.max(2, Math.min(proposed || 2, capacity.usableForExecution));
  }
  ...
  async routeHumanRequest(human: Agent, input: HumanMessageInput, rawMode: string | undefined,
    ...
    const brains = channel.memberIds.map(id => this.hive.getAgent(id)).filter(a => a.role === 'brain');
    if (channel.type !== 'dm' || !channel.memberIds.includes(HUMAN_ID) || brains.length !== 1)
      throw new HiveError(400, 'Adaptive topology requires a Human-to-brain DM');
    ...
    return this.queued(channel.id, async () => {
      ...
      const config = loadAdaptiveRouting(this.hive.home);
      const inherited = manual ? null : this.conversationLock(channel.id);
      if (!config?.enabled && mode === 'auto' && !inherited) return null;
      const previous = this.row(channel.id);
      // Do not silently transfer the preceding execution's workers to a new Human root.
      if (previous && this.capacity(previous).activeTasks > 0)
        throw new HiveError(409, 'The current execution still has delegated work; continue in its thread or finish it before a new request');
      ...
      let capacity = this.capacity({ projectId: channel.projectId, brainId: brain.id, executionId });
      const project = this.hive.getProject(channel.projectId);
      ...
      let decision = config?.enabled ? await evaluateAdaptiveTopology(makeSnapshot(), config)
        : fallbackDecision(locked ?? 'single', minimumTopologyWorkers(locked ?? 'single'));
      for (let attempt = 0; attempt < 2 && decision.providerStatus === 'ok'; attempt++) {
        const fresh = this.capacity({ projectId: channel.projectId, brainId: brain.id, executionId });
        if (JSON.stringify(fresh.workers.available) === JSON.stringify(capacity.workers.available)) { capacity = fresh; break; }
        capacity = fresh;
        decision = await evaluateAdaptiveTopology(makeSnapshot(), config!);
      }
      const fresh = this.capacity({ projectId: channel.projectId, brainId: brain.id, executionId });
      capacity = fresh;
      ...
      if (!locked && uncertain) actual = config?.fallback === 'single' ? 'single'
        : config ? this.feasibleFallback(config, capacity.workers) : 'single';
      if (!locked && mode === 'orchestrated_auto' && actual === 'single' && capacity.workers.usableForExecution > 0)
        actual = config ? this.feasibleFallback(config, capacity.workers) : 'brain_one_worker';
      const required = minimumTopologyWorkers(actual);
      if (manual && required > capacity.workers.usableForExecution)
        throw new HiveError(409, `${manual} needs ${required} available workers; only ${capacity.workers.usableForExecution} are available`);
      let budget = this.manualBudget(actual, decision.targetWorkers, capacity.workers);
      ...
      if (required > capacity.workers.usableForExecution || budget > capacity.workers.usableForExecution) {
        desired = { topology: actual, workers: required };
        actual = 'single'; budget = 0;
      ...
      if (!locked && capacity.workers.usableForExecution === 0 && (decision.needsOrchestration || mode === 'orchestrated_auto')) {
        desired = { topology: config?.topologyFallback ?? 'brain_one_worker', workers: minimumTopologyWorkers(config?.topologyFallback ?? 'brain_one_worker') };
        warning = 'Orchestration needed · no workers available';
      }
      const policy = initialTopologyPolicy({ topology: actual, workers: budget });
      const state: StoredExecution = {
        executionId, channelId: channel.id, projectId: channel.projectId, brainId: brain.id, rootMessageId: '',
        currentTopology: actual, workerBudget: budget,
        desiredTopology: desired?.topology ?? null, desiredWorkers: desired?.workers ?? null,
        lockedTopology: locked, lockScope, orchestratedOnly: mode === 'orchestrated_auto',
        providerAvailable: decision.providerStatus !== 'unavailable', warning, recommendation: decision,
        confirmations: 0, confirmationHighCount: 0, confirmationTopology: null, confirmationWorkers: null,
        eventsSinceChange: policy.eventsSinceChange, updatedAt: Date.now(), recentEvents: [], revision: 1,
      };
      let item!: AdaptiveRoutingEvent;
      const delivered = this.hive.postAdaptiveRequest(human, input, topologyDirective(state), `adaptive-${executionId}`,
        persistReceipt, message => {
          state.rootMessageId = message.id;
          if (manual && scope === 'conversation') this.conversationLockWrite(channel.id, manual);
          this.save(state);
          item = this.record(state, decision, actual, warning ? 'warning' : locked ? 'lock' : 'evaluation', false);
          this.saveEvent(item);
        });
  ...
  setLock(actor: Agent, channelId: string, raw: unknown): AdaptiveRoutingView {
    ...
    if (Object.keys(input).some(key => !['scope', 'topology'].includes(key)) || typeof input.scope !== 'string')
      throw new HiveError(400, 'Expected an explicit lock scope');
    ...
    const decision = { ...fallbackDecision(topology ?? state.currentTopology, state.desiredWorkers ?? state.workerBudget),
      reason: scope === 'none' ? 'human_unlock' : 'human_lock' };
