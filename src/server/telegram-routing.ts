...
export function initTelegramRouting(db: DatabaseSync, legacyNamespace: string): void {
  ...
  outboxTransaction(db, () => {
    ...
    if (db.prepare("SELECT 1 FROM telegram_routing_migrations WHERE version = 1").get()) return;
    const shapes = {
      telegram_in: {
      ...
      telegram_hold: {
        ...
        sql: "telegram_chat_id INTEGER NOT NULL, telegram_message_id INTEGER NOT NULL, telegram_thread_id INTEGER NOT NULL, payload TEXT NOT NULL, project_id TEXT, PRIMARY KEY(bot_key, telegram_chat_id, telegram_message_id)",
      },
    };
