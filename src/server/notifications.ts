import type { NotificationHost } from './services/ports.ts';
...
export class NotificationStore {
  ...
  constructor(private hive: NotificationHost) {
    this.lookup = hive.storage.db.prepare(`SELECT event_types FROM notification_subscriptions
  ...
  private scope(actor: Agent, raw: unknown) {
    ...
    if (parsed.data.threadId) {
      const root = this.hive.storage.db.prepare('SELECT channel_id, thread_id FROM messages WHERE id = ?').get(parsed.data.threadId);
  ...
  list(actor: Agent): Subscription[] {
    if (actor.role !== 'brain' && actor.role !== 'worker') throw new HiveError(403, 'Only agents have wake subscriptions');
    return this.hive.storage.db.prepare(`SELECT channel_id, thread_id, event_types FROM notification_subscriptions
  ...
  set(actor: Agent, raw: unknown) {
    ...
    this.hive.storage.db.prepare(`INSERT INTO notification_subscriptions(agent_id, channel_id, thread_id, event_types) VALUES (?, ?, ?, ?)
  ...
  reset(actor: Agent, raw: unknown) {
    ...
    this.hive.storage.db.prepare('DELETE FROM notification_subscriptions WHERE agent_id = ? AND channel_id = ? AND thread_id = ?')
