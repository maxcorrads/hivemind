...
test('notification wakes recheck authorization without hydrating a full roster for each recipient', async t => {
  ...
  const local = hive.identity.join({ role: 'worker', seniority: 'mid' }).agent;
  const room = hive.channels.createChannel(brain, { name: 'local-work', type: 'private', memberNames: [local.name] });
  const foreignProject = hive.projects.createProject(human, { name: 'Other', slug: 'other' });
  const foreign = hive.identity.join({ role: 'worker', seniority: 'mid', project: foreignProject.slug }).agent;
  ...
  let channelLoads = 0;
  const mock = t.mock.method(hive.channels, 'getChannel', (...args: Parameters<Hive['channels']['getChannel']>) => {
    channelLoads++;
    return original(...args);
  });
  const sent = hive.messages.postMessage(brain, { channel: room.id, body: 'Visible local work', eventType: 'assignment' });
  mock.mock.restore();
  ...
  assert.equal(hive.delivery.isFor(foreign, sent), false);
  assert.throws(() => hive.messageQueries.getVisibleMessage(foreign, sent.seq), /Cannot read/);
  const batch = await hive.delivery.wait(local, 1);
  assert.deepEqual(batch.delivery!.messageSeqs, [sent.seq]);
  removeChannelMember(hive, room.id, local.id);
  await assert.rejects(hive.delivery.wait(local, 1), /no longer accessible/);
