...
test("shutdown has a deadline even for a stalled request header", async t => {
  const { hive } = fixture(t);
  const started = startServer({ port: 0, hive, telegram: false, shutdownGraceMs: 50 });
  const port = await started.ready;
  ...
  await assert.rejects(started.shutdown(), /deadline/);
  await closed;
  assert.equal(hive.getAgent("human").role, "human");
});
