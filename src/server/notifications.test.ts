...
function fixture(t: TestContext, fakeClock = false) {
  ...
  const brain = hive.identity.join({ role: 'brain' }), peer = hive.identity.join({ role: 'brain' });
  const worker = hive.identity.join({ role: 'worker', seniority: 'mid' });
  const room = hive.channels.createChannel(brain.agent, { name: 'work', type: 'private', memberNames: [peer.agent.name, worker.agent.name] });
  markInboxRead(hive);
  const session = hive.delivery.openInboxSession(brain.agent, crypto.randomUUID());
  const wait = (ms = 1) => hive.delivery.wait(brain.agent, ms, undefined, { sessionId: session, compact: true });
  const ack = (mail: WaitResult) => hive.delivery.acknowledgeInbox(brain.agent, session, mail.delivery!.id);
  const send = (body: string, eventType?: Message['eventType'], threadId?: string) =>
    hive.messages.postMessage(worker.agent, { channel: room.id, body, eventType, threadId });
...
test('subscriptions persist, scope to self and never grant private/cross-project access', async t => {
  const f = fixture(t), general = f.hive.channels.getChannel('general', f.brain.agent.projectId);
  f.hive.notifications.set(f.brain.agent, { channel: general.id, eventTypes: ['blocker'] });
  const post = (eventType?: Message['eventType']) => f.hive.messages.postMessage(f.worker.agent, { channel: general.id, body: 'Observation', eventType });
  const quiet = post('progress'), alert = post('blocker'), legacy = post();
  assert.equal(f.hive.delivery.isFor(f.brain.agent, quiet), false);
  assert.equal(f.hive.delivery.isFor(f.peer.agent, alert), false);
  assert.equal(f.hive.delivery.isFor(f.brain.agent, legacy), false);
  ...
  const outsider = f.hive.identity.join({ role: 'brain' });
  assert.throws(() => f.hive.notifications.set(outsider.agent, { channel: f.room.id, eventTypes: ['message'] }), /access/);
  f.hive.projects.createProject(f.hive.identity.getAgent('human'), { name: 'Other', slug: 'other' });
  const other = f.hive.identity.join({ role: 'brain', project: 'other' });
  ...
  const bot = f.hive.bots.createBot(f.hive.identity.getAgent('human'), f.room.projectId, { name: 'ObservationBot' });
  ...
  assert.equal(f.hive.delivery.isFor(f.brain.agent, post('blocker')), false);
});
...
test('thread overrides channel, directed mail bypasses filters and reset restores inherited defaults', async t => {
  ...
  const direct = f.hive.messages.postMessage(f.worker.agent, { channel: f.room.id, body: 'Target', recipients: [f.brain.agent.name], eventType: 'decision' });
  const mention = f.send(`@${f.brain.agent.name} Question`, 'question');
  assert.equal(f.hive.delivery.isFor(f.peer.agent, direct), false);
  ...
  assert.deepEqual(f.hive.messageQueries.getMessageById(direct.id).recipientIds, [f.brain.agent.id]);
  f.hive.notifications.reset(f.brain.agent, { channel: f.room.id, threadId: root.id });
  assert.equal(f.hive.delivery.isFor(f.brain.agent, f.send('Inherited mute', 'blocker', root.id)), false);
  f.hive.notifications.reset(f.brain.agent, { channel: f.room.id });
  assert.equal(f.hive.delivery.isFor(f.brain.agent, f.send('Default', undefined, root.id)), true);
...
test('aged receipt totals survive progress batching, mute, session replacement and pending replay', async t => {
  ...
  f.hive.notifications.set(f.brain.agent, { channel: f.room.id, eventTypes: [] });
  const session = f.hive.delivery.openInboxSession(f.brain.agent, crypto.randomUUID());
  assert.throws(() => f.ack(first), /superseded/);
  f.reopen(); forbidAggregation();
  const replay = await f.hive.delivery.wait(f.brain.agent, 1, undefined, { sessionId: session, compact: true });
  ...
  assert.equal(f.hive.inbox.status(f.brain.agent.id).acknowledgedMessages, historical);
  f.hive.delivery.acknowledgeInbox(f.brain.agent, session, replay.delivery!.id);
  assert.equal(f.hive.delivery.acknowledgeInbox(f.brain.agent, session, replay.delivery!.id).duplicate, true);
  assert.equal(f.hive.delivery.inboxStatuses()[f.brain.agent.id].acknowledgedMessages, historical + 2);
  f.send('Muted after the offered batch', 'progress', root.id);
  const quiet = f.hive.delivery.wait(f.brain.agent, 1, undefined, { sessionId: session, compact: true });
...
test('structured tasks notify participants, not unrelated room peers, with optional observer subscription', async t => {
  const f = fixture(t);
  const peerSession = f.hive.delivery.openInboxSession(f.peer.agent, crypto.randomUUID());
  ...
  assert.equal(f.hive.delivery.isFor(f.worker.agent, assigned.message), true);
  assert.equal(f.hive.delivery.isFor(f.peer.agent, assigned.message), false);
  assert.equal((await f.hive.delivery.wait(f.peer.agent, 1, undefined, { sessionId: peerSession })).idle, true);
  ...
  const peerMail = await f.hive.delivery.wait(f.peer.agent, 1, undefined, { sessionId: peerSession });
...
test('acknowledgement-only chatter creates no wake loop, including mentions; evidence and Human decisions remain deliverable', async t => {
  ...
  const file = await f.hive.files.createFileFromBytes(f.worker.agent, { name: 'evidence.txt', mime: 'text/plain', bytes: new TextEncoder().encode('fixture') });
  const evidence = f.hive.messages.postMessage(f.worker.agent, { channel: f.room.id, body: 'Receipt with evidence', eventType: 'acknowledgement', attachmentIds: [file.id] });
  const human = f.hive.messages.postMessage(f.hive.identity.getAgent('human'), { channel: f.room.id, body: 'Proceed', eventType: 'decision' });
  const delivered = await f.wait(); assert.deepEqual(delivered.delivery!.messageSeqs, [evidence.seq, human.seq]);
  assert.equal(f.hive.messageQueries.listMessages(f.brain.agent, f.room.id, { limit: 100 }).messages.filter(m => m.eventType === 'acknowledgement').length, 21);
});
...
test('fixed progress window coalesces same-thread mail for one channel; blockers and Human decisions bypass it', async t => {
  ...
  assert.equal(f.hive.messageQueries.expandDigest(f.brain.agent, mail.mail![0].expand).messages.length, 2); f.ack(mail);
  for (const author of [f.worker.agent, f.hive.identity.getAgent('human')]) {
    const active = f.wait(1000); f.send('Batch next progress', 'progress', root.id);
    t.mock.timers.tick(10);
    const urgent = f.hive.messages.postMessage(author, { channel: f.room.id, body: 'Need decision now',
...
test('cancellation and session replacement clean up routine timers without reserving unsent mail', async t => {
  ...
  const old = f.hive.delivery.wait(f.brain.agent, 1000, abort.signal, { sessionId: f.session });
  ...
  const nextSession = f.hive.delivery.openInboxSession(f.brain.agent, crypto.randomUUID()); await rejection;
  t.mock.timers.tick(ROUTINE_BATCH_MS);
  assert.equal(f.hive.inbox.pending(f.brain.agent.id), undefined);
  const mail = await f.hive.delivery.wait(f.brain.agent, 1000, undefined, { sessionId: nextSession });
...
test('round-robin conversation delivery and reserved critical slots preserve ACK holes under continuous noise', async t => {
  ...
  const readerSession = f.hive.delivery.openInboxSession(f.worker.agent, crypto.randomUUID());
  const root = f.hive.messages.postMessage(f.brain.agent, { channel: f.room.id, body: 'Busy task' });
  const expected = [root.seq];
  for (let i = 0; i < 32; i++) expected.push(f.hive.messages.postMessage(f.brain.agent, { channel: f.room.id, threadId: root.id, body: 'Busy status' }).seq);
  const quiet = f.hive.messages.postMessage(f.brain.agent, { channel: f.room.id, body: 'Quiet task' }); expected.push(quiet.seq);
  const urgent = f.hive.messages.postMessage(f.brain.agent, { channel: f.room.id, body: 'Blocked', eventType: 'blocker' }); expected.push(urgent.seq);
  const seen: number[] = [];
  for (let batch = 0; seen.length < expected.length; batch++) {
    assert.ok(batch < 12, 'No task may starve behind newer noise');
    const result = await f.hive.delivery.wait(f.worker.agent, 1000, undefined, { sessionId: readerSession, compact: true });
    ...
    f.hive.delivery.acknowledgeInbox(f.worker.agent, readerSession, result.delivery!.id);
    if (batch < 3) expected.push(f.hive.messages.postMessage(f.brain.agent, { channel: f.room.id, body: 'New busy status', threadId: root.id }).seq);
  }
...
test('explicit Human recipients populate For you on live ingress, snapshot and reload, respecting project and read state', async t => {
  ...
  const human = f.hive.identity.getAgent('human'), general = f.hive.channels.getChannel('general', f.brain.agent.projectId);
  ...
  const alsoMentioned = f.hive.messages.postMessage(f.brain.agent, { channel: f.room.id,
    body: '@Human Another question', recipients: ['Human'], eventType: 'question' });
  const elsewhere = f.hive.projects.createProject(human, { name: 'Other notifications', slug: 'other-notifications' });
  const otherBrain = f.hive.identity.join({ role: 'brain', project: elsewhere.slug });
  const other = f.hive.messages.postMessage(otherBrain.agent, { channel: 'general', body: 'Other decision', recipients: ['Human'] });
  ...
  assert.deepEqual(f.hive.reads.mentionInbox(human, 30, undefined, f.room.projectId).messages.map(m => m.id), [alsoMentioned.id, sent.id]);
  assert.deepEqual(f.hive.reads.mentionInbox(human, 30, alsoMentioned.seq, f.room.projectId).messages.map(m => m.id), [sent.id]);
  ...
  f.hive.reads.markRead(human, general.id, sent.seq);
  assert.deepEqual(f.hive.reads.mentionInbox(human).messages.map(m => m.id), [other.id, alsoMentioned.id]);
  ...
  assert.deepEqual(f.hive.reads.mentionInbox(human).messages.map(m => m.id), [other.id]);
});
...
test('attachment-only messages can target Human without permitting worker-to-Human messages', async t => {
  ...
  const file = await f.hive.files.createFileFromBytes(f.brain.agent, { name: 'report.txt', mime: 'text/plain', bytes: new TextEncoder().encode('fixture report') });
  const sent = f.hive.messages.postMessage(f.brain.agent, { channel: f.room.id, body: '', recipients: ['Human'], attachmentIds: [file.id] });
  const inbox = f.hive.reads.mentionInbox(human).messages;
  ...
  for (const recipients of [['Human'], [f.brain.agent.name, 'hUmAn']])
    assert.throws(() => f.hive.messages.postMessage(f.worker.agent, { channel: f.room.id, body: 'Request', recipients }), /permitted person/);
  assert.deepEqual(f.hive.reads.mentionInbox(human).messages.map(m => m.id), [sent.id]);
});

test('directed progress stays immediate and full, retaining intended recipients through replay', async t => {
  const f = fixture(t, true);
  const original = f.hive.messages.postMessage(f.worker.agent, { channel: f.room.id, body: 'Direct progress',
    eventType: 'progress', recipients: [f.brain.agent.name] });
  f.hive.messages.postMessage(f.worker.agent, { channel: f.room.id, body: 'Direct progress two', threadId: original.id,
    eventType: 'progress', recipients: [f.brain.agent.name] });
...
test('subscription cleanup follows project deletion without leaving stale channel rules', t => {
  ...
  f.hive.projects.deleteProject(f.hive.identity.getAgent('human'), f.room.project);
...
test('controlled task replay measures model-returning waits, not idle HTTP polls', async t => {
  ...
  const run = async (legacyPolicy: boolean) => {
    ...
    f.hive.bus.on('message', (m: Message) => { if (f.hive.delivery.isFor(f.brain.agent, m)) addressedWakeSignals++; });
    ...
    assert.equal(f.hive.messageQueries.listMessages(f.brain.agent, f.room.id, { threadId: taskId, limit: 100 }).messages.length, 20);
