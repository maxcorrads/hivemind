...
// Exercise the real authenticated server rather than weakening its Human gate
// to accommodate a fixture written before local UI authentication was added.
test("Hono dependency preserves live HTTP routes, JSON errors and authenticated WebSocket events", { timeout: 20_000 }, async (t) => {
  ...
  assert.ok(cookie, "Human bootstrap must establish a session cookie");
