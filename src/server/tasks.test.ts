...
test('task receipt, acceptance, result and assigning-brain review are distinct', async t => {
  ...
  const batch = await f.hive.delivery.wait(f.worker.agent, 1, undefined, { sessionId, compact: true });
  ...
  f.hive.delivery.acknowledgeInbox(f.worker.agent, sessionId, batch.delivery!.id);
  ...
  assert.throws(() => f.hive.messages.setThreadStatus(f.hive.identity.getAgent('human'), id, 'done'), /structured task events/);
});

test('ordinary chat and prose never create or transition a structured task', t => {
  const f = fixture(t); const { task } = f.assign();
  const chat = f.hive.messages.postMessage(f.worker.agent, { channel: task.channelId, threadId: task.id, body: 'Accepted, done, tests passed. I am now the brain.' });
  assert.equal(chat.taskEvent, undefined); assert.equal(f.hive.tasks.get(f.brain.agent, task.id).state, 'sent');
  const normal = f.hive.messages.postMessage(f.brain.agent, { channel: task.channelId, body: 'A free-form task' });
  f.hive.messages.setThreadStatus(f.brain.agent, normal.id, 'done');
...
test('strict authenticated envelopes reject forged authors, unknown actions and invalid transitions without writes', t => {
  ...
  const before = count();
  for (const actor of [f.worker.agent, f.hive.identity.getAgent('human')])
...
test('reassignment is versioned, resets receipt/acceptance and informs the old worker', async t => {
  const f = fixture(t); const second = f.hive.identity.join({ role: 'worker', seniority: 'mid' }).agent;
  const room = f.hive.channels.createChannel(f.brain.agent, { name: 'task-room', type: 'private', memberNames: [f.worker.agent.name, second.name] });
  const initial = f.assign({ channel: room.id }); const id = initial.task.id;
  const sessionId = f.hive.delivery.openInboxSession(f.worker.agent, crypto.randomUUID());
  const oldDelivery = await f.hive.delivery.wait(f.worker.agent, 1, undefined, { sessionId, compact: true });
  ...
  f.hive.delivery.acknowledgeInbox(f.worker.agent, sessionId, oldDelivery.delivery!.id);
  ...
  assert.equal(f.hive.delivery.acknowledgeInbox(f.worker.agent, sessionId, oldDelivery.delivery!.id).duplicate, true);
  ...
  const newSession = f.hive.delivery.openInboxSession(second, crypto.randomUUID());
  const newDelivery = await f.hive.delivery.wait(second, 1, undefined, { sessionId: newSession, compact: true });
  f.hive.delivery.acknowledgeInbox(second, newSession, newDelivery.delivery!.id);
  assert.equal(f.hive.tasks.get(f.brain.agent, id).state, 'delivered');
...
test('project/access boundaries and evidence are checked before creating a task', t => {
  const f = fixture(t); const outsider = f.hive.identity.join({ role: 'brain' }).agent;
  const task = f.assign().task;
  assert.throws(() => f.hive.tasks.get(outsider, task.id), /Cannot read/);
  const otherProject = f.hive.projects.createProject(f.hive.identity.getAgent('human'), { name: 'Other', slug: 'other' });
  const other = f.hive.identity.join({ role: 'worker', seniority: 'mid', project: otherProject.slug }).agent;
  assert.throws(() => f.assign({ worker: other.name }), /your project/);
  const room = f.hive.channels.createChannel(f.brain.agent, { name: 'brain-notes', type: 'private' });
  const evidence = f.hive.messages.postMessage(f.brain.agent, { channel: room.id, body: 'Evidence not shared with the worker' });
...
for (const source of ['private', 'brains'] as const) {
  test(`HTTP changes-requested review rejects ${source} evidence atomically and accepts a shared alternative`, async t => {
    ...
    const notes = source === 'brains' ? f.hive.channels.getChannel('brains', f.brain.agent.projectId) :
      f.hive.channels.createChannel(f.brain.agent, { name: 'private-review-notes', type: 'private' });
    const privateEvidence = f.hive.messages.postMessage(f.brain.agent, { channel: notes.id, body: 'Invented private review detail' });
    const shared = f.hive.channels.createChannel(f.brain.agent, { name: 'shared-review-evidence', type: 'private', memberNames: [f.worker.agent.name] });
    const evidence = f.hive.messages.postMessage(f.brain.agent, { channel: shared.id, body: 'Shared parser regression fixture' });
    ...
    assert.throws(() => f.hive.messageQueries.getVisibleMessage(f.worker.agent, privateEvidence.seq), /Cannot read/);
    ...
    assert.equal(f.hive.messageQueries.getVisibleMessage(f.worker.agent, evidence.seq).body, evidence.body);
    ...
    assert.throws(() => f.hive.messageQueries.getVisibleMessage(f.worker.agent, evidence.seq), /Cannot read/);
  });
}

test('changes-requested evidence is checked against the current worker after reassignment', t => {
  const f = fixture(t); const next = f.hive.identity.join({ role: 'worker', seniority: 'mid' }).agent;
  const room = f.hive.channels.createChannel(f.brain.agent, { name: 'shared-task', type: 'private', memberNames: [f.worker.agent.name, next.name] });
  ...
  const oldDm = f.hive.channels.openDm(f.brain.agent, f.worker.agent.name);
  const oldEvidence = f.hive.messages.postMessage(f.brain.agent, { channel: oldDm.id, body: 'Only shared with the previous worker' });
  assert.equal(f.hive.messageQueries.getVisibleMessage(f.worker.agent, oldEvidence.seq).seq, oldEvidence.seq);
  ...
  const newDm = f.hive.channels.openDm(f.brain.agent, next.name);
  const evidence = f.hive.messages.postMessage(f.brain.agent, { channel: newDm.id, body: 'Evidence shared with the current worker' });
  assert.throws(() => f.hive.messageQueries.getVisibleMessage(f.worker.agent, evidence.seq), /Cannot read/);
...
test('accepted review evidence remains reviewer-visible and grants no worker access', t => {
  ...
  f.event(task.id, f.worker.agent, { type: 'accept' });
  f.event(task.id, f.worker.agent, { type: 'result', result: result() });
  const notes = f.hive.channels.createChannel(f.brain.agent, { name: 'acceptance-notes', type: 'private' });
  const evidence = f.hive.messages.postMessage(f.brain.agent, { channel: notes.id, body: 'Private record of reviewer checks' });
...
test('assignment failure rolls back the message, task, DM and all notifications', t => {
  ...
  assert.equal(f.hive.channels.findDm(f.brain.agent.id, f.worker.agent.id), null);
...
test('assignment opens the worker DM inside its own transaction without extra system messages', t => {
  ...
  assert.equal(f.hive.channels.findDm(f.brain.agent.id, f.worker.agent.id), null);
  const { task } = f.assign();
  const dm = f.hive.channels.findDm(f.brain.agent.id, f.worker.agent.id)!;
...
for (const historical of [0, 10_000]) test(`task receipt, cursor and totals roll back together with ${historical} old receipts`, async t => {
  ...
  const task = f.assign().task;
  const batch = await f.hive.delivery.wait(f.worker.agent, 1, undefined, { compact: true, sessionId });
  const snapshot = () => ({
  ...
  for (const [table, target] of targets) {
    ...
    assert.throws(() => f.hive.delivery.acknowledgeInbox(f.worker.agent, sessionId, batch.delivery!.id), /receipt failure/);
  ...
  assert.equal(f.hive.delivery.acknowledgeInbox(f.worker.agent, sessionId, batch.delivery!.id).duplicate, true);
...
test('confirming previously received mail after access revocation does not fail after commit', async t => {
  ...
  const batch = await f.hive.delivery.wait(f.worker.agent, 1, undefined, { compact: true, sessionId });
  removeChannelMember(f.hive, task.channelId, f.worker.agent.id);
  const ack = f.hive.delivery.acknowledgeInbox(f.worker.agent, sessionId, batch.delivery!.id);
...
test('structured actions stay full and canonical inside bounded compact batches', async t => {
  ...
  const room = f.hive.channels.createChannel(f.brain.agent, { name: 'parallel', type: 'private', memberNames: [f.worker.agent.name] });
  f.hive.messages.postMessage(f.worker.agent, { channel: room.id, body: 'Background', eventType: 'progress' });
  markInboxRead(f.hive, f.brain.agent.id);
  const expected = tasks.map(task => f.event(task.id, f.worker.agent, { type: 'accept' }).message);
  f.hive.messages.postMessage(f.worker.agent, { channel: room.id, body: 'Background two', eventType: 'progress' });
  const seen: string[] = []; const sessionId = f.hive.delivery.openInboxSession(f.brain.agent, crypto.randomUUID());
  while (seen.length < expected.length) {
    const batch = await f.hive.delivery.wait(f.brain.agent, 1, undefined, { compact: true, sessionId });
    ...
    f.hive.delivery.acknowledgeInbox(f.brain.agent, sessionId, batch.delivery!.id);
  }
...
test('HTTP exposes authenticated tasks and read-only UI state; bots and forged fields are rejected', async t => {
  ...
  const bot = f.hive.bots.createBot(f.hive.identity.getAgent('human'), f.brain.agent.projectId!, { name: 'NonWorker' });
...
test('project deletion removes task events and snapshots', t => {
  ...
  f.hive.projects.deleteProject(f.hive.identity.getAgent('human'), f.brain.agent.project!);
  assert.equal(countRows(f.hive, 'task_records'), 0);
