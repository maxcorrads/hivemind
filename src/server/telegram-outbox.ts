...
export type TelegramJob = { seq: number; kind: "message" | "reaction"; botKey: string | null; chatId: number | null };
...
export function telegramJob(db: DatabaseSync, seq: number, kind: string): TelegramJob | undefined {
  return db.prepare(
    "SELECT seq, kind, bot_key AS botKey, telegram_chat_id AS chatId FROM telegram_pending WHERE seq = ? AND kind = ?",
