...
for (const legacy of [false, true]) {
  test(`scoped indexes support ${legacy ? "pre-project upgrade" : "fresh install"} and restart`, (t) => {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      try {
        for (const index of ["idx_agents_project_role", "idx_channels_project_type_name"]) {
...
test("real wait admits and counts overflow using the scoped channel lookup", async (t) => {
  ...
  while (received.length < expected.length) {
    const page = await hive.wait(worker, 100);
    assert.equal(page.idle, false);
    hadOverflow ||= (page.more ?? 0) > 0;
    received.push(...page.messages.map((m) => m.id));
  }
  assert.equal(hadOverflow, true);
