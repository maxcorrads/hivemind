...
function fixture(t: TestContext) {
  ...
  const room = hive.channels.createChannel(brain.agent, { name: "parallel-work", type: "private", memberNames: [worker.agent.name] });
  markInboxRead(hive, brain.agent.id);
  const sessionId = hive.delivery.openInboxSession(brain.agent, crypto.randomUUID());
  const send = (body: string, eventType?: Message["eventType"], threadId?: string, channel = dm.id) =>
    hive.messages.postMessage(worker.agent, { channel, body, eventType, threadId });
  const other = () => send("Other task", "progress", undefined, room.id);
  const wait = () => hive.delivery.wait(brain.agent, 1, undefined, { compact: true, sessionId });
...
test("digests separate roots and authors and include exact IDs, sequence bounds and root references", async t => {
  ...
  const secondWorker = f.hive.identity.join({ role: "worker", seniority: "mid" });
  f.hive.channels.invite(f.brain.agent, f.room.id, [secondWorker.agent.name]);
  const roomRoot = f.send("Room update", "progress", undefined, f.room.id);
  const otherAuthor = f.hive.messages.postMessage(secondWorker.agent, { channel: f.room.id, threadId: roomRoot.id, body: "Different author", eventType: "progress" });
  const result = await f.wait(); bounded(result);
  for (const messages of expected) {
    ...
    assert.deepEqual(f.hive.messageQueries.expandDigest(f.brain.agent, digest.expand).messages.map(m => m.id), messages.map(m => m.id));
  }
...
test("a first-message compact result can start a reply without a history scan", async t => {
  ...
  const reply = f.hive.messages.postMessage(f.brain.agent, { channel: item.channelId, threadId: item.rootId, body: "Here is the clarification" });
  ...
  const workerSession = f.hive.delivery.openInboxSession(f.worker.agent, crypto.randomUUID());
  const workerMail = await f.hive.delivery.wait(f.worker.agent, 1, undefined, { compact: true, sessionId: workerSession });
...
test("exact digest expansion paginates after ACK and restart without including interleaved or newer mail", async t => {
  ...
  const expected = [root];
  for (let i = 1; i < 35; i++) {
    f.hive.messages.postMessage(f.brain.agent, { channel: f.dm.id, threadId: root.id, body: "Interleaved instruction" });
  ...
  const first = f.hive.messageQueries.expandDigest(f.brain.agent, digest.expand); bounded(first);
  ...
  f.hive.delivery.acknowledgeInbox(f.brain.agent, f.sessionId, batch.delivery!.id);
  ...
  let page = first;
  while (page.hasMore) {
    page = f.hive.messageQueries.expandDigest(f.brain.agent, { ...digest.expand!, afterSeq: page.nextAfterSeq });
  ...
  assert.deepEqual(f.hive.messageQueries.expandDigest(f.brain.agent, { ...digest.expand!, afterSeq: expected.at(-1)!.seq }),
...
test("expansion rejects invisible, mixed-channel, missing and invalid references without altering receipts", async t => {
  ...
  assert.throws(() => f.hive.messageQueries.expandDigest(outsider.agent, reference), /Cannot read/);
  assert.throws(() => f.hive.messageQueries.expandDigest(f.brain.agent, { ...reference, messageIds: [message.id, other.id] }), /outside this channel/);
  assert.throws(() => f.hive.messageQueries.expandDigest(f.brain.agent, { ...reference, messageIds: [crypto.randomUUID()] }), /missing/);
  for (const change of [{ messageIds: [] }, { messageIds: [message.id, message.id] }, { messageIds: ["not-an-id"] },
    ...
    assert.throws(() => f.hive.messageQueries.expandDigest(f.brain.agent, { ...reference, ...change }));
  f.hive.projects.createProject(f.hive.identity.getAgent("human"), { name: "Isolated", slug: "isolated" });
  const outside = f.hive.identity.join({ role: "brain", project: "isolated" });
  assert.throws(() => f.hive.messageQueries.expandDigest(outside.agent, reference), /Channel not found/);
...
test("attachment-bearing progress stays full and expansion returns metadata without file contents", async t => {
  const f = fixture(t);
  const file = await f.hive.files.createFileFromBytes(f.worker.agent, { name: "evidence.txt", mime: "text/plain", bytes: new TextEncoder().encode("private fixture file bytes") });
  const original = f.hive.messages.postMessage(f.worker.agent, { channel: f.dm.id, body: "Evidence", eventType: "progress", attachmentIds: [file.id] });
  ...
  const expanded = f.hive.messageQueries.expandDigest(f.brain.agent, { channel: f.dm.id, messageIds: [original.id] });
...
test("typed bot observations survive replay/restart and do not acquire command authority", async t => {
  const f = fixture(t);
  const bot = f.hive.bots.createBot(f.hive.identity.getAgent("human"), f.room.projectId, { name: "ReportBot" });
  f.hive.channels.invite(f.brain.agent, f.room.id, [bot.bot.name]);
  const payload = { eventId: "event-1", body: "Permission denied", eventType: "blocker" };
  const sent = f.hive.bots.postBotMessage(bot.bot, f.room.id, payload).message;
  f.hive.bots.postBotMessage(bot.bot, f.room.id, { eventId: "event-2", threadId: sent.id, body: "Thanks", eventType: "progress" });
  ...
  assert.equal(f.hive.bots.postBotMessage(bot.bot, f.room.id, payload).duplicate, true);
  assert.throws(() => f.hive.bots.postBotMessage(bot.bot, f.room.id, { ...payload, eventType: "progress" }), /already used/);
  const legacy = { eventId: "untyped", body: "Legacy sender" };
  f.hive.bots.postBotMessage(bot.bot, f.room.id, legacy);
  f.reopen();
  assert.equal(f.hive.bots.postBotMessage(bot.bot, f.room.id, legacy).duplicate, true);
  ...
  assert.equal(f.hive.messageQueries.getVisibleMessage(f.brain.agent, sent.seq).eventType, "blocker");
});
...
test("digest references and expanded Unicode/control-character bodies fit the existing byte budget", async t => {
  ...
  while (received < originals.length + 2) {
    ...
    for (const item of batch.mail ?? []) if (item.expand) {
      ...
      do {
        const page = f.hive.messageQueries.expandDigest(f.brain.agent, { ...item.expand, ...(afterSeq === undefined ? {} : { afterSeq }) });
        bounded(page);
        for (const message of page.messages) {
          assert.equal(message.body, f.hive.messageQueries.getVisibleMessage(f.brain.agent, message.seq).body);
  ...
  const page = f.hive.messageQueries.expandDigest(f.brain.agent, refs); bounded(page);
...
test("oversized legacy originals fail explicitly without skipping them or changing inbox state", async t => {
  ...
  const before = f.hive.inbox.status(f.brain.agent.id);
  const page = f.hive.messageQueries.expandDigest(f.brain.agent, reference); bounded(page);
  assert.equal(page.hasMore, true);
  assert.deepEqual(page.messages.map(m => m.id), [first.id]);
  assert.equal(page.nextAfterSeq, first.seq);
  assert.throws(() => f.hive.messageQueries.expandDigest(f.brain.agent, { ...reference, afterSeq: page.nextAfterSeq }),
