...
test("real joins reserve 500 identities across projects, offline state and restart", (t) => {
  ...
  const human = hive.identity.getAgent(HUMAN_ID);
  const alpha = hive.projects.createProject(human, { name: "Alpha", slug: "names-alpha" });
  const beta = hive.projects.createProject(human, { name: "Beta", slug: "names-beta" });
  const joined: Array<ReturnType<Hive["identity"]["join"]>> = [];
  const taken = new Set<string>();

  for (let index = 0; index < 500; index += 1) {
    const project = index % 4 < 2 ? alpha.slug : beta.slug;
    const result = index % 2 === 0
      ? hive.identity.join({ role: "brain", project })
      : hive.identity.join({ role: "worker", seniority: "mid", project });
    ...
    joined.push({ ...result, agent: hive.identity.getAgent(result.agent.id) });
    hive.identity.setOffline(result.agent.id);
  }

  assert.deepEqual(taken, new Set(AGENT_NAMES.map((name) => name.toLowerCase())));
  const first = joined[0]!;
  const other = joined[2]!;
  const visible = hive.identity.listAgents(first.agent);
  ...
  assert.equal(hive.identity.getAgent(HUMAN_ID).name, HUMAN_NAME);
  assert.equal(hive.identity.getAgent(HUMAN_ID).role, "human");

  assert.throws(() => hive.identity.join({ role: "brain", token: first.token, project: beta.slug }), /project cannot change/);
  assert.throws(
    () => hive.identity.join({ role: "brain", token: first.token, resumeName: other.agent.name }),
    /not /,
  );
  assert.equal(hive.identity.listAgents().length, 501);

  const numberedBrain = hive.identity.join({ role: "brain", project: alpha.slug });
  const numberedWorker = hive.identity.join({ role: "worker", seniority: "mid", project: beta.slug });
  assert.equal(numberedBrain.agent.name, "Brain1");
  assert.equal(numberedWorker.agent.name, "Worker1");
  hive.identity.setOffline(numberedBrain.agent.id);
  hive.identity.setOffline(numberedWorker.agent.id);

  fixture.close(hive);
  hive = fixture.open();
  for (const original of joined) {
    assert.equal(hive.identity.getAgent(original.agent.id).name, original.agent.name, "restart renamed an identity");
  }
  const resumed = hive.identity.join({
  ...
  const resumedNumber = hive.identity.join({
  ...
  assert.equal(hive.identity.listAgents().length, 503);
  assert.equal(hive.identity.join({ role: "brain", project: alpha.slug }).agent.name, "Brain2");
  assert.equal(hive.identity.join({ role: "worker", seniority: "mid", project: beta.slug }).agent.name, "Worker2");
});
...
test("interleaved joins on two database handles reserve names in production", (t) => {
  ...
  const taken = new Set<string>();
  for (let index = 0; index < 500; index += 1) {
    const result = handles[index % 2]!.identity.join({ role: "brain" });
  ...
  assert.equal(handles[0]!.identity.join({ role: "brain" }).agent.name, "Brain1");
  assert.equal(handles[1]!.identity.join({ role: "brain" }).agent.name, "Brain2");
});
