...
function fixture(t: TestContext) {
  ...
  const hive = new Hive(file), human = hive.identity.getAgent("human");
...
test("send keys bind exact payloads atomically and replay attachments after restart", async t => {
  ...
  const attachment = await f.hive.files.createFileFromBytes(f.brain.agent, { name: "proof.txt", mime: "text/plain", bytes: new TextEncoder().encode("fixture") });
  ...
  const original = f.hive.messages.postMessage(f.brain.agent, payload);
  assert.equal(f.hive.messages.postMessage(f.brain.agent, payload).id, original.id);
  ...
  for (const patch of [{ body: "changed" }, { attachmentIds: [] }, { eventType: "blocker" as const }, { recipients: [f.worker.agent.name] }])
    assert.throws(() => f.hive.messages.postMessage(f.brain.agent, { ...payload, ...patch }), /already used/);
  for (const requestId of ["", "../escape", "x".repeat(101)])
    assert.throws(() => f.hive.messages.postMessage(f.brain.agent, { ...input, requestId }), /Invalid requestId/);
  assert.equal(countRows(f.hive, "send_requests"), 1);
  f.hive.db.close();
  const restart = new Hive(f.file); t.after(() => restart.db.close());
  const replay = restart.messages.postMessage(f.brain.agent, payload);
...
test("a journal insert failure rolls back message/bindings/events and leaves the key reusable", async t => {
  const f = fixture(t), attachment = await f.hive.files.createFileFromBytes(f.brain.agent,
  ...
  assert.throws(() => f.hive.messages.postMessage(f.brain.agent, input), /ledger failure/);
  ...
  restoreLedger();
  f.hive.messages.postMessage(f.brain.agent, input);
...
test("keys remain project scoped, cannot bypass visibility and do not evict unexpired guarantees", t => {
  ...
  const original = f.hive.messages.postMessage(f.brain.agent, input);
  const other = f.hive.projects.createProject(f.human, { name: "Other", slug: "other" });
  const foreign = f.hive.identity.join({ role: "brain", project: other.slug });
  assert.throws(() => f.hive.messages.postMessage(foreign.agent, input), /cannot|not found/i);
  const one = f.hive.messages.postMessage(f.human, { channel: "general", body: "A", requestId: "human-key" });
  const two = f.hive.messages.postMessage(f.human, { channel: f.hive.channels.getChannel("general", other.id).id, body: "B", requestId: "human-key" });
  ...
  assert.throws(() => f.hive.messages.postMessage(f.brain.agent, { ...input, requestId: "over-cap" }), /full/);
  assert.equal(f.hive.messages.postMessage(f.brain.agent, input).id, original.id, "old key remains replayable at capacity");
  backdate(f.hive, "send_requests", "expires_at", { actor_id: f.brain.agent.id });
  const afterWindow = f.hive.messages.postMessage(f.brain.agent, input);
...
test("desired-state reactions are retry safe, transactional and permission checked", t => {
  const f = fixture(t), message = f.hive.messages.postMessage(f.brain.agent, { channel: f.dm.id, body: "react" });
  let events = 0; f.hive.bus.on("reaction", () => events++);
  assert.equal(f.hive.messages.setReaction(f.worker.agent, message.seq, "👍", true).added, true);
  assert.equal(f.hive.messages.setReaction(f.worker.agent, message.seq, "👍", true).added, true);
  assert.equal(events, 1);
  assert.equal(f.hive.messages.setReaction(f.worker.agent, message.seq, "👍", false).added, false);
  assert.equal(f.hive.messages.setReaction(f.worker.agent, message.seq, "👍", false).added, false);
  assert.equal(events, 2);
  assert.equal(f.hive.messages.toggleReaction(f.worker.agent, message.seq, "👍").added, true);
  assert.equal(f.hive.messages.toggleReaction(f.worker.agent, message.seq, "👍").added, false);
  assert.throws(() => f.hive.messages.setReaction(f.worker.agent, 1.5, "👍", true), /Invalid/);
  assert.throws(() => f.hive.messages.setReaction(f.worker.agent, message.seq, "bogus", true), /Invalid/);
  assert.throws(() => f.hive.messages.setReaction(f.worker.agent, message.seq, "👍", "true" as unknown as boolean), /Invalid/);
  const foreign = f.hive.identity.join({ role: "brain" }).agent;
  assert.throws(() => f.hive.messages.setReaction(foreign, message.seq, "👍", true), /Cannot/);
});
