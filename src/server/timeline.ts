...
export class TimelineStore {
  ...
  recordMessage(messageId: string, input: { source?: 'hive' | 'telegram' | 'bot'; traceId?: string | null; causeMessageId?: string | null } = {}) {
    const row = this.deps.storage.db.prepare('SELECT id, thread_id, created_at FROM messages WHERE id = ?').get(messageId) as
  ...
  private header(seq: number) {
    return this.deps.storage.db.prepare(`SELECT m.id, m.seq, m.channel_id, COALESCE(m.thread_id,m.id) AS root_id,
  ...
  private wakeReason(actor: Agent, seq: number): string {
    ...
    const subscription = this.deps.storage.db.prepare(`SELECT event_types FROM notification_subscriptions
  ...
  trace(actor: Agent, traceId: string, taskId: string | null = null): TimelineView {
    ...
    const rows = this.deps.storage.db.prepare(`SELECT m.id,m.seq,m.channel_id,m.author_id,m.body,m.event_type,m.created_at,
    ...
    for (const row of rows.slice(0, TIMELINE_EVENT_LIMIT)) {
      ...
      const messageEvent: TimelineMessageEvent = {
        ...
        authorRole:String(row.author_role ?? 'unknown'), source:(provenance?.source ?? (this.deps.storage.db.prepare('SELECT 1 FROM bot_events WHERE message_id=?').get(row.id) ? 'bot' : 'hive')) as any,
      ...
      const deliveries = this.deps.storage.db.prepare(`SELECT d.*,a.name,a.role FROM timeline_deliveries d
  ...
  prune(now=Date.now()) {
    ...
    const old=this.deps.storage.db.prepare(`DELETE FROM message_provenance AS p WHERE created_at<? AND NOT ${active}`).run(cutoff).changes;
    const deliveryOld=this.deps.storage.db.prepare(`DELETE FROM timeline_deliveries WHERE last_offered_at<?
    ...
    const provExcess=Number(this.deps.storage.db.prepare('SELECT MAX(COUNT(*)-?,0) AS n FROM message_provenance').get(TIMELINE_MAX_PROVENANCE_ROWS)!.n);
    if (provExcess>0) this.deps.storage.db.prepare(`DELETE FROM message_provenance WHERE message_id IN (
      SELECT p.message_id FROM message_provenance p WHERE NOT ${active} ORDER BY p.created_at LIMIT ?)`).run(provExcess);
    const delExcess=Number(this.deps.storage.db.prepare('SELECT MAX(COUNT(*)-?,0) AS n FROM timeline_deliveries').get(TIMELINE_MAX_DELIVERY_ROWS)!.n);
    if (delExcess>0) this.deps.storage.db.prepare(`DELETE FROM timeline_deliveries WHERE rowid IN (
