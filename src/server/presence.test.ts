...
test("presence writes coalesce 10,000 authenticated touches at the exact heartbeat boundary", (t) => {
  ...
  const f = fixture(t);
  const agent = f.hive.join({ role: "worker", seniority: "mid" }).agent;
  f.events.length = 0;
  const before = f.writes();
  for (let i = 0; i < 10_000; i++) f.hive.touch(agent.id);
  assert.equal(f.writes(), before);
  assert.equal(f.events.length, 0);
  at += 14_999;
  f.hive.touch(agent.id);
  assert.equal(f.writes(), before);
  at += 1;
  f.hive.touch(agent.id);
  assert.equal(f.writes(), before + 1);
  assert.equal(f.hive.getAgent(agent.id).lastSeenAt, at);
  assert.equal(f.events.length, 0);
  f.hive.touch(agent.id);
  assert.equal(f.writes(), before + 1);
});
...
test("online/offline oscillation emits immediately and redundant offline calls are no-ops", (t) => {
  ...
  f.hive.setOffline("human");
  assert.equal(f.hive.getAgent("human").online, true);
  assert.equal(f.writes(), before + 8);
});
...
test("presence coalescing and deletion stay independent across projects and survive restart", (t) => {
  ...
  const first = f.hive.listProjects()[0]!;
  const second = f.hive.createProject(human, { name: "Other", slug: "other" });
  const a = f.hive.join({ role: "worker", seniority: "mid", project: first.slug }).agent;
  at += 1_000;
  const b = f.hive.join({ role: "worker", seniority: "mid", project: second.slug }).agent;
  f.events.length = 0;
  at += 14_000;
  const before = f.writes();
  f.hive.touch(a.id);
  ...
  assert.equal(f.events.length, 0);
  f.reopen();
  assert.equal(f.hive.getAgent(a.id).lastSeenAt, at);
  assert.equal(f.hive.getAgent(b.id).lastSeenAt, at - 14_000);
  const afterRestart = f.writes();
  f.hive.touch(a.id);
  assert.equal(f.writes(), afterRestart);
  f.hive.setOffline(a.id);
  f.hive.deleteProject(human, first.slug);
  const afterDelete = f.writes();
  assert.doesNotThrow(() => f.hive.touch(a.id));
  assert.doesNotThrow(() => f.hive.setOffline("does-not-exist"));
  assert.equal(f.writes(), afterDelete);
  assert.equal(f.hive.getAgent(b.id).online, true);
  assert.deepEqual(f.hive.listAgents(b).map((agent) => agent.id).sort(), ["human", b.id].sort());
});
...
test("real WebSocket connections share one ordered stream and hello gives the reconnect baseline", async (t) => {
  ...
  f.hive.bus.emit("agent", f.hive.getAgent("human"));
