...
function saveJSON(home: string, name: string, value: unknown) {
  ...
  try {
    ...
    // A successful rename consumes the temporary file. On failure, retain the
    // original cause even if removing the temporary file also fails.
    try { unlinkSync(temporary); }
    catch (cleanupError) {
      if ((cleanupError as NodeJS.ErrnoException).code !== "ENOENT")
        throw new AggregateError([error, cleanupError], "Plugin save and temporary-file cleanup failed");
    }
    throw error;
  }
}
...
export const configurePlugin: Configure = (executable, profile, request) =>
  new Promise((resolve, reject) => {
    ...
    const data = (d: Buffer, stdout: boolean) => {
      bytes += d.length;
      if (bytes > 65536) {
        failed = true;
        stop();
      } else if (stdout) out += d;
    };
    child.stdout.on("data", (d) => data(d, true));
    child.stderr.on("data", (d) => data(d, false));
    child.stdin.on("error", () => {});
    child.on("error", () => {
      clearTimeout(timer);
      reject(
        new Error("Could not run the installed plugin configuration command"),
      );
    });
    child.on("close", (code) => {
      clearTimeout(timer);
      if (failed || code !== 0) {
        let detail =
          "Plugin rejected configuration. Check its local CLI validation.";
        if (!failed)
          try {
            ...
            if (
              ...
              detail = receipt.error.slice(0, 300);
          } catch {}
      ...
      try {
        ...
        reject(new Error("Invalid plugin configuration receipt"));
      }
    });
    ...
    return {
      ...
      pluginError:
        error instanceof Error
          ? error.message
          : "Could not load project plugins",
    };
  }
}
