...
test("Vite builds on the test runtime and serves React with real API and WS proxies", { timeout: 45_000 }, async (t) => {
  ...
  t.after(async () => {
    ...
    try {
      ...
      if (server) {
        backendClosed = server.shutdown().then(() => t.diagnostic("backend closed"));
      }
  ...
  // Bootstrap through the real proxy. Host and Origin must remain the browser
  // authority while the session cookie is namespaced by the backend listener.
  const session = await fetch(`${base}/api/ui/session`, {
    method: "POST", headers: { origin: base, "content-type": "application/json" },
    body: "{}", signal: t.signal,
  });
  assert.equal(session.status, 200);
  const cookie = session.headers.get("set-cookie")?.split(";", 1)[0];
  assert.ok(cookie, "proxied bootstrap must establish a Human session");
  await session.body?.cancel();
  socket = new WebSocket(`ws://127.0.0.1:${address.port}/ws`, { headers: { origin: base, cookie } });
